...
/// @title Bubble Sort Fee V3 — Continuous spike + bubble sort adaptive scale
/// @notice Hybrid approach: uses proven continuous proportional spike mechanism,
///         but the spike SCALE itself is adaptive — a small bubble-sorted array
///         of scale multipliers self-organizes based on toxic flow feedback.
///         Scales that are too low (still attract arbs) bubble UP, scales that
///         deter arbs effectively bubble DOWN (becoming the default).
contract Strategy is AMMStrategyBase {
    uint256 public constant NUM_SCALES = 5;

    // Storage layout
    uint256 private constant SLOT_SCALES = 0;       // slots 0-4: spike scale values
    uint256 private constant SLOT_SCORES = 5;       // slots 5-9: toxic scores per scale
    ...
    uint256 private constant SLOT_CONSEC_ARB = 14;  // consecutive arb trades

    // Base parameters (proven from V8)
    ...
    function afterInitialize(uint256, uint256) external override returns (uint256, uint256) {
        // Initialize spike scale candidates (bubble sort will reorder these)
        // The "active" scale is always index 0 (the one that bubbled to lowest toxic score)
        writeSlot(SLOT_SCALES + 0, 35 * BPS);   // default (proven V8 value)
        writeSlot(SLOT_SCALES + 1, 25 * BPS);   // conservative
        writeSlot(SLOT_SCALES + 2, 30 * BPS);   // moderate
        writeSlot(SLOT_SCALES + 3, 40 * BPS);   // aggressive
        writeSlot(SLOT_SCALES + 4, 50 * BPS);   // very aggressive
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        // Get current active spike scale (index 0 after bubble sort)
        ...
        if (size > threshold && threshold > 0) {
            ...
            // Proportional spike using the active (bubble-sorted) scale
            ...
            // Record toxic score: higher score = this scale wasn't enough to deter arb
            // Use index based on how many consecutive arbs (round-robin through scales)
            ...
            // Retail: reward the active scale (low toxic score = good)
        ...
        // Periodic bubble sort: scales with HIGH toxic scores bubble UP (index increases)
        // The scale at index 0 is the "best" (lowest toxic attraction)
        if (tradeCount % SORT_INTERVAL == 0) {
            for (uint256 i = 0; i < NUM_SCALES - 1; i++) {
                ...
                // If scale i attracted MORE toxic flow than i+1, swap them
                if (sI > sJ) {
