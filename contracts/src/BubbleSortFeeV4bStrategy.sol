...
/// @title BubbleSortFee V4b — scale=60, max=200
contract Strategy is AMMStrategyBase {
    ...
    uint256 public constant MAX_SPIKE = 200 * BPS;
    ...
    function afterInitialize(uint256, uint256) external override returns (uint256, uint256) {
        writeSlot(SLOT_SCALES + 0, 60 * BPS);
        writeSlot(SLOT_SCALES + 1, 45 * BPS);
        writeSlot(SLOT_SCALES + 2, 50 * BPS);
        writeSlot(SLOT_SCALES + 3, 75 * BPS);
        writeSlot(SLOT_SCALES + 4, 90 * BPS);
    ...
    function getName() external pure override returns (string memory) { return "BubbleSortV4b"; }
}
