...
/// @title BubbleSortFee V4u — scale=45, decay=WAD/4, base=20 (faster decay with 45 scale)
contract Strategy is AMMStrategyBase {
    ...
    uint256 public constant MAX_SPIKE = 250 * BPS;
    uint256 public constant ALPHA = WAD / 10;
    uint256 public constant DECAY_SPEED = WAD / 4;
    ...
    function afterInitialize(uint256, uint256) external override returns (uint256, uint256) {
        writeSlot(SLOT_SCALES + 0, 45 * BPS);
        writeSlot(SLOT_SCALES + 1, 35 * BPS);
        writeSlot(SLOT_SCALES + 2, 40 * BPS);
        writeSlot(SLOT_SCALES + 3, 55 * BPS);
        writeSlot(SLOT_SCALES + 4, 65 * BPS);
    ...
    function afterSwap(TradeInfo calldata trade) external override returns (uint256, uint256) {
        ...
        if (volEma == 0) { volEma = size; }
        else { volEma = (ALPHA * size + (WAD - ALPHA) * volEma) / WAD; }
        ...
        if (size > threshold && threshold > 0) {
            ...
            if (score > WAD / 4) { writeSlot(SLOT_SCORES + 0, score - WAD / 4); }
            else { writeSlot(SLOT_SCORES + 0, 0); }
        }
    ...
    function getName() external pure override returns (string memory) { return "BubbleSortV4u"; }
}
