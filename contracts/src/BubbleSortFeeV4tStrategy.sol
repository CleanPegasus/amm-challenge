...
/// @title BubbleSortFee V4t — scale=50, decay=WAD/5, base=20
contract Strategy is AMMStrategyBase {
    ...
    function afterInitialize(uint256, uint256) external override returns (uint256, uint256) {
        writeSlot(SLOT_SCALES + 0, 50 * BPS);
        writeSlot(SLOT_SCALES + 1, 40 * BPS);
        writeSlot(SLOT_SCALES + 2, 45 * BPS);
        writeSlot(SLOT_SCALES + 3, 60 * BPS);
        writeSlot(SLOT_SCALES + 4, 70 * BPS);
    ...
    function getName() external pure override returns (string memory) { return "BubbleSortV4t"; }
}
